...
class MenuElementController extends Controller
{
    ...
    {
        $element = MenuItem::where('id', '=', $request->input('id'))->first();
        $switchElement = MenuItem::where('menus_id', '=', $element->menus_id)
    ...
    {
        $menuId = $request->input('menu');
        $result = MenuItem::select('menus.name as label', 'menus.id as value')
            ->where('menus.menus_id', '=', $menuId)
    ...
    public function create()
    {
        return response()->json([
            ...
            'menulist' => Menu::select('menulist.name as label', 'menulist.id as value')->get(),
        ]);
    }
    ...
    {
        $result = MenuItem::select('menus.sequence')
            ->where('menus.menus_id', '=', $menuId)
    ...
    public function store(Request $request)
    {
        ...
        $menus->menus_id = $request->input('menu');
    ...
    public function edit(Request $request)
    {
        return response()->json([
            'roles' => RolesService::get(),
            'menulist' => Menu::select('menulist.name as label', 'menulist.id as value')->get(),
            'menuElement' => MenuItem::where('id', '=', $request->input('id'))->first(),
    ...
    {
        $validatedData = $request->validate($this->getValidateArray());
        $menus = MenuItem::where('id', '=', $request->input('id'))->first();
        $menus->slug = $request->input('type');
        $menus->menus_id = $request->input('menu');
        $menus->icon = $request->input('icon');
        $menus->href = $request->input('href');
    ...
    {
        $menuElement = MenuItem::join('menus as mparent', 'menus.parent_id', '=', 'mparent.id')
            ->join('menulist', 'menus.menus_id', '=', 'menulist.id')
        ...
        if (empty($menuElement)) {
            $menuElement = MenuItem::join('menulist', 'menus.menu_id', '=', 'menulist.id')
